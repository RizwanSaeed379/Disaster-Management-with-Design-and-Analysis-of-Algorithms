...
struct DisasterManager {
    ...
    // ==========================================
    // ALTERNATIVE: Maximum Demand Fulfillment
    // Call this if you want to maximize total delivered demand
    // ==========================================
    void allocateMaximizeDemand() {
        vector<Node> nodes;
        for (const Node& n : graph.nodes)
            if (n.id != 0) nodes.push_back(n);

        // Sort by demand (descending) to pack large items first
        sort(nodes.begin(), nodes.end(), [](const Node& a, const Node& b) {
        ...
        buildRoutes(assignedNodes);
    }

    // ==========================================
    // ALTERNATIVE: Efficiency-Based Allocation
    // Maximizes priority per unit capacity
    // ==========================================
    void allocateEfficiency() {
        vector<Node> nodes;
        for (const Node& n : graph.nodes)
            if (n.id != 0) nodes.push_back(n);

        // Sort by priority/demand ratio, break ties by priority
        sort(nodes.begin(), nodes.end(), [](const Node& a, const Node& b) {
            double ratioA = a.demand > 0 ? (double)a.priority / a.demand : (double)a.priority;
            double ratioB = b.demand > 0 ? (double)b.priority / b.demand : (double)b.priority;
            if (abs(ratioA - ratioB) > 0.001) return ratioA > ratioB;
        ...
        // Try to fit high-efficiency nodes
        for (auto& node : nodes) {
            if (nodeAssigned[node.id]) continue;

            for (size_t i = 0; i < vehicles.size(); ++i) {
                if (remainingCap[i] >= node.demand) {
                    assignedNodes[i].push_back(node.id);
                    remainingCap[i] -= node.demand;
                    nodeAssigned[node.id] = true;
                    break;
                }
            }
